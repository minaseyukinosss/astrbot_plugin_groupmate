...
对照目标 bot 的行为分布（见 `analysis/target_bot_20260824/report.md`）：连续对话 48.4%、被点名或被回复 27.1%、自主定向回复 12.4%、环境话题加入 **11.5%**、主动开场 0.1%。

**目标 bot 的环境介入本身只占 11.5%。** 调松 AMBIENT 阈值不会让 bot 更像群友，只会让它变成一个乱插话的机器人。当前 silence 率高不是缺陷。
