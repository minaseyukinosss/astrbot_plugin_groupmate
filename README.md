...
AstrBot 4.24 及以上版本会在插件详情页显示 Groupmate 的“控制中心”页面。首页先展示运行状态、影子模式和待标注数量；进入“审阅”后可以连续查看影子决策并标注为“必须回复、可以回复、必须沉默”或“跳过”。

页面中的暂停/恢复只切换当前插件实例的运行状态，不会改写 `_conf_schema.json` 配置；插件重载后是否启用影子模式等行为仍以配置页为准。已标注记录可以从页面下载为脱敏评测集，页面不会展示 QQ 号、群号、HMAC 或数据库路径。
