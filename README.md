...
- 关系驱动的主动关心：只跟进考试、健康、搬家、求职等重要经历，并在关系一般、事项敏感、群聊繁忙或安静时段选择克制
...
- `schema v20（当前数据库版本）` 在人格隔离账本上继续保存未完事项、自然跟进、自我承诺与主动关心判断；同一群和同一用户在不同人格下不会共享短期窗口或长期状态。
...
1. **插件 Pages（本插件页）**：WebUI → 插件 →「群聊伙伴 Groupmate」详情 → 打开 **settings** 页。可查看总览、参与、成员、关系、记忆、关心、决策、质量与审计；“关心”会展示对象、重要经历、关系与现场依据、开口或沉默原因，并允许纠正错误关联。顶栏可暂停/恢复。
