...
- Delivery workload 把 1,000 个结构化 `SENDING` parts 交给生产 `OneBotDeliveryAdapter`，但其底层 callback 只返回本地 fake receipt、绝不联网。报告从实际 `DeliveryReceiptStatus` accumulator 导出 attempted/sent/unknown。
...
1. Event loss 非零：保持 SHADOW/OFF，定位 ingest/commit 缺口；不得用调高 backlog 或 latency 预算掩盖丢失。
2. Actor backlog 超限：先检查 Ambient 聚合、输入尖峰和 DB busy，不通过丢事件降压。
3. Worker concurrency 超限：视为 hard-cap 实现故障，停止发布；不能只提高文档预算。
4. Worker cost 或 latency 超限：降低 worker 选择/成本或增加经验证的容量，再重跑；Fast 不得排在 Ambient backlog 后。
5. Ambient P99/expiry 超限：保持 SHADOW；runtime 在 deadline guardrail 处持久化 explicit discard，不得为过期 scene 创建 capture/outbox。
6. Projection lag 超限：修复或重建 Projection。重建必须同时重放 SSE event view；Actor、Task 和 Outbox 主链必须继续，不能为追 Projection 回滚事实。
