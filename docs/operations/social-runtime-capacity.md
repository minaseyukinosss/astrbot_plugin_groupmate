...
- 每个虚拟秒为每群记入 5 个事件；`ingested == committed == 450000`，`dropped == 0`。
- 每群每 30 秒有一个确定性的 FAST 直接事件，共 3,000 个 FAST denominator；群号错峰，避免用单批常量伪造延迟。
- 持续 Ambient 流量按 5 秒有界窗口聚合，共 18,000 个 Ambient decision denominator；单群窗口最多 25 个事件。
- 10 个 30 秒长任务在起始区间重叠。默认 hard cap 12 时实测长任务并发峰值必须为 10，总 Worker 并发峰值不得超过 12。
- 虚拟 Worker 使用生产 `WorkerAdmissionQueue`。FAST 排在已经等待的 AMBIENT 之前，但不会中断已运行工作；报告公开 `direct_admitted_ahead_of_ambient` 和 `direct_starvation_count`。
...
| Worker cost | 50 cost units / virtual second | FAST=1、Ambient=2、长任务=2 的总成本除以 1,800 |
| Fast decision P95 | ≤ 2,500 ms | 3,000 个 FAST decision；不包含外部 Task duration |
| Ambient decision P95 | ≤ 8,000 ms | 18,000 个聚合 Ambient decision |
...
4. Projection lag 超限：修复或重建 Projection。Actor、Task 和 Outbox 主链必须继续，不能为追 Projection 回滚事实。
5. Unknown delivery 出现：保持 `UNKNOWN` 并人工核查平台证据，绝不盲重试。
