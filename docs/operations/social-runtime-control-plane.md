...
游戏知识默认启用，但当前只运行本地语义解析和群约定后台学习。预装范围为三角洲行动、鸣潮、崩坏：星穹铁道和绝区零。原神不再预装，但仍可像其他非预装游戏一样由群聊证据学习；升级时只退休其旧 bundled 投影和定时任务，不删除群内已学习知识。`knowledge_web_search_enabled` 在本阶段只被严格解析和保存，不创建搜索客户端、不发起 HTTP 请求，也不意味着已具备新版本事实。
