...
首版预装原神、三角洲行动、鸣潮、崩坏：星穹铁道和绝区零的稳定语义包。五款游戏的官方版本状态以每 24 小时完成一次成功核验为目标；失败任务只重试和上报降级，不伪造成功时间。用户提到新版本、前瞻、爆料、刚更新等时效表达时，根据参与通道执行即时核验。
...
1. 五款 seed 游戏固定评测中的高频话题理解正确率达到 95%。
2. 歧义实体的高置信错误归并率低于 1%。
3. 群内约定跨群泄漏为零。
