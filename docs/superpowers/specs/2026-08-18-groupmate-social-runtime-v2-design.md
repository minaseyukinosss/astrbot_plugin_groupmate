...
平台消息还必须携带结构化的交互所有权事实：

- `SOCIAL_RUNTIME`：Groupmate 自己作出的社会参与或能力调用；
- `EXTERNAL_PLUGIN`：用户通过明确命令、固定触发器或自动解析规则调用的其他插件；
- `PLATFORM`：平台和系统行为；
- `UNKNOWN`：宿主没有提供足够事实，且没有命中管理员确认的规则。

所有权与发送账号相互独立。同一个 QQ 账号发出的消息可能由 Groupmate、其他
AstrBot 插件或平台功能产生，禁止仅凭 `is_self` 把它们都归为 Groupmate 行为。
`EXTERNAL_PLUGIN` 输入仍进入 Group Scene 作为发生过的上下文，但不得建立社会回复
义务、启动注意力 Worker、影响关系亲密度或被计为 Groupmate 的参与。宿主命令路由
和停止传播事实优先；对不遵守宿主传播约定的第三方插件，仅可使用管理员明确配置的
精确前缀、固定触发器和链接域名规则兜底。不得解析其他 bot 的输出文本猜测所有权、
Provider 状态或任务结果。
...
Groupmate 使用 Capability Catalog 知道当前 AstrBot 实例真正可用的外置功能。Catalog
只包含已经安装、具有明确 Adapter、通过 Contract 校验并由管理员加入 allowlist 的
能力；模型只能选择 `capability_id` 和类型化参数，不能编造插件名或向群里发送命令
文本来冒充用户调用。优先调用插件提供的结构化 API；只有经过专门适配、权限语义等价
且结果可结构化捕获时，旧式命令 Handler 才能成为 Provider。

调用所有权按每次 invocation 决定：

- 用户发送 `xw...`、`ww...`、`bq...` 或链接等明确触发器时，外置插件拥有请求和
  返回；Groupmate 默认只观察，不重复回复；
- Groupmate 根据场景主动选择 allowlist 能力时，Social Runtime 拥有社会决策、
  授权、TaskRun 和最终可见交付，外置插件仅拥有受约束的执行；
- 用户随后以普通语言继续提问会形成新的社会决策点，不继承前一命令的回复抑制。

默认仅允许自主调用只读或低影响能力。账号登录/绑定、群管理、禁言、配置修改、消费、
敏感和破坏性操作不得自主调用，或必须按 Capability Contract 获取相应权限与明确确认。
参考群聊中观察到的插件或指令只用于历史语料分类，不构成目标 Bot 的安装清单、依赖或
默认 allowlist；目标 Bot 的 Catalog 必须以它自己的部署实例为准。
...
历史导出缺少可靠的插件路由元数据时，允许用管理员确认的参考触发规则标记
`REFERENCE_EXTERNAL_TRIGGER`，但该标记只能表达“像外置功能”，不能声称目标 Bot
安装了同名插件。外置功能焦点不得进入核心社会参与指标；含外置功能的上下文可以保留，
但相应输入和输出必须带所有权注释。来源为 `UNKNOWN` 的输出从效果分母排除并保留安全
扫描。评估报告分为三个互不混算的 lane：核心社会对话、Groupmate 主动能力调用、外置
插件兼容性（不抢答、不重复、不误计）。
