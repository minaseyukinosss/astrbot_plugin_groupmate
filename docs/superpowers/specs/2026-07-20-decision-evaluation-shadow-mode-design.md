...
当前学习素材 `c000001.jsonl` 包含 17,972 条消息，但导出统计显示消息全部来自同一个 Bot。该数据可以用于：

- 回复长度、句数和常见表达统计；
- 客服腔、系统旁白和重复表达回归测试；
- 构建人格风格示例候选集。

该数据不能用于判断 Bot 当时为什么开口，也不能计算主动介入精确率或沉默准确率，因为缺少开口前的完整群友消息窗口。

介入评测必须使用以下任一来源：

- 人工编写并审核的最小场景；
- 包含多成员上下文的完整群聊导出；
- 影子模式采集后经过脱敏和人工标注的真实窗口。

## 6. 总体架构

```text
                         ┌────────────────────┐
标准评测 JSONL ─────────→│ OfflineReplayRunner │
                         └─────────┬──────────┘
                                   │ ChatMessage
                                   ▼
                           TriggerRouter
...
NapCat 实时事件 ─→ GroupActor ─→ ShadowDecisionProbe ─→ ShadowRecord
                                      │
                                      └── 禁止生成、视觉和发送
```
...
9. 使用真实决策 Provider 执行一次离线对比。
10. 在测试群运行影子模式，采集待人工标注窗口。
