...
- 固定短提示必须染上爱弥斯风格：保持轻、短、有一点个人感，但只能说明“复制出来的 @ 不算有效呼叫”，不能进入正常闲聊、不能读取好感、不能根据上下文生成。

这保留复制文本 @ 的特殊处理，同时避免它污染新决策机制。示例口径：`复制出来的 @ 不算数哦，要叫爱弥斯的话，用真正的 @。`。实施时可按 Persona Pack（人格提示词包）微调具体措辞，但必须是固定模板或有限模板，不能调用模型自由生成。
...
覆盖：复制文本 @ 不进入 `ParticipationDecisionEngine`（统一参与决策引擎）、不产生 `DIRECT_REQUIRED`（明确点名必答）、不增加 `DirectAddressPressure`（直接呼叫压力）、不读取好感档位、不开启续聊；短窗口重复复制文本 @ 时沉默或只返回爱弥斯风格固定提示。
...
5. 复制文本 @ 的提示保留原特殊处理，但文案由 `CopiedAtTipHandler`（复制 @ 提示处理器）固定输出为爱弥斯风格短句，不调用模型生成；
6. 明确点名不会被开放参与预算或旧回退开关取消，生成失败时仍有最小人格化回应；
7. 普通群消息必须有明确 `ParticipationMotive`（人格参与动机）才参与；
8. `AffinityBand`（好感档位）影响开放参与类型和直接回应姿态，但不变成分数加成；
9. 过度 @ 被 `DirectAddressPressure`（直接呼叫压力）识别，并按好感档位输出边界、应声或戏谑；
10. 爱弥斯仍保持爱弥斯人格，不复制小维身份、口癖、素材或媒体；
11. 所有新增文档中的函数名和关键字都配中文说明；
12. 单元测试和影子评估能解释每一个主要回复/沉默差异。
