...
- [x] 写失败测试：bot 自己生成的回复不能证明用户事实；敏感候选默认 REVIEW；冲突事实并存且标记；删除后等价文本被 tombstone 阻断；召回不跨群。
...
- [x] Commit `feat: complete social cognition shadow runtime`，使用 `superpowers:requesting-code-review`；通过后进入 Phase C。
